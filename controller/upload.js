        ...
        try {
          ...
          for (let i = 0; i < dataArray.length; i++) {
            const doc = await clientModel.updateMany(
              { orderid: dataArray[i].orderid },
              { $set: { awb: dataArray[i].awb } }
            );
            console.log(doc);
            docFinal.push(doc);
          }
  ...
  try {
      ...
      fs.createReadStream(filePath)
        ...
        .on("data", (rowData) => {
          ...
          const extractedData = {
            ...
            amount: (parseFloat(orderTotalMap[orderid]) / occurrence).toFixed(
              2
            ),
          ...
          if (
            (rowData["Tags"] && rowData["Tags"].includes("COD")) ||
            (rowData["Payment Method"] &&
              rowData["Payment Method"].includes("COD"))
          ) {
    ...
    try {
      ...
      if (insertError.code === 11000) {
        console.log(
          "Duplicate key error occurred. Some records were not inserted:",
          insertError
        );
        res
          .status(202)
          .send(
            "CSV file uploaded and processed with warnings for duplicate keys."
          );
      } else {
        throw insertError;
      }
    ...
    processCSV(
      ...
      async (rowData) => {
        ...
        if (existingEntry && existingEntry.status != "return_recieved") {
          ...
          existingEntry &&
          existingEntry.status === "return_recieved"
        ) {
          return;
        } else {
          const extractedData = {
  ...
  try {
    ...
    processCSV(
      ...
      async (rowData) => {
        ...
        let date =
          rowData["Settlement Date"] != "NA" && rowData["Settlement Date"];
        ...
        let orderStatus;
        if (
          rowData["Order Status"] == "HANDOVER" ||
          rowData["Order Status"] == "PRINT"
        ) {
          orderStatus = "ready_to_ship";
        } else if (
          rowData["Order Status"] == "RTO_ACKNOWLEDGED" ||
          rowData["Order Status"] == "RTO_DELIVERED"
        ) {
          orderStatus = "return_delivered";
        ...
        if (existingEntry && existingEntry.status != "return_recieved") {
          ...
          function splitAddress(address) {
            ...
            const reversedRestOfAddressWithState =
              restOfReversedAddress.replace(reversedCity + " ,", "");
            const reversedRestOfAddress =
              reversedRestOfAddressWithState.replace(reversedState + " ,", "");
...
exports.updateDate = async (req, res) => {
  try {
    ...
    const updateResult = await sampleClientModel.updateMany(
      ...
      } // Convert using $dateFromString
    );

    console.log(
      `${updateResult.modifiedCount} documents updated successfully.`
    );
    client.close();
  } catch (error) {
    console.error(error);
